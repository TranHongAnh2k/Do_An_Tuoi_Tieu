...
void setup()
{
  ...
  {
    delay(500);
    if (mqtt.connect("ESP8266_100", username, password))
      break;
  }
...
void reconnect()
{
  ...
  while (WiFi.status() != WL_CONNECTED)
  {
    ...
    WiFi.reconnect();
    mqtt.connect("ESP8266_100", username, password);
    delay(5000);
    // chờ để kết nối lại
...
//  // push data to mqtt
  ...
  mqtt.publish(("ESPg/RL1"+MacAddress).c_str(), String(digitalRead(RL1pin)).c_str());
  mqtt.publish(("ESPg/RL2"+MacAddress).c_str(), String(digitalRead(RL2pin)).c_str());
  mqtt.publish(("ESPg/RL3"+MacAddress).c_str(), String(digitalRead(RL3pin)).c_str());
  mqtt.publish(("ESPg/RL4"+MacAddress).c_str(), String(digitalRead(RL4pin)).c_str());
}
